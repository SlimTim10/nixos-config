  ...
  (cl-loop
   (cond
    ...
    ((and				; If possible configuration
      ;; Compatibility with perspective mode
      ;; Only use the configuration if all its buffers belong to this perspective
      (let* ((buf-ring (ring-ref winner-pending-undo-ring winner-undo-counter))
			 (bufs (mapcar 'cdr (cdr buf-ring))))
		(seq-every-p #'persp-is-current-buffer bufs))
      ; try changing the windows
